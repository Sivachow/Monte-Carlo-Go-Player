import numpy as np
import os, sys
from board_util import GoBoardUtil, BLACK, WHITE, EMPTY, BORDER, coord_to_point, PASS
...
class Feature(object):
    ...
    def find_full_board_features(features, board):
        ...
        for a in anchors:
            ...
            # if (
            #     num_lib == 1 and GoBoardUtil.opponent(board.current_player) == color
            # ):  # find capture feature
            #     #theLib = liberty_dic[a][0]
            #     #Feature.find_capture_features(features, board, a, theLib)
            if (
                num_lib == 2 and GoBoardUtil.opponent(board.current_player) == color
            ):  # find atari feature
